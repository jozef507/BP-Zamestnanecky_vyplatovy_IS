...
class Levy extends CI_Controller
{
	public function index()
	{
		...
		$resp = $this->LevyMod->get_levies();
		json_output(200,$resp);
	}

	public function crt_levy()
	{
		$method = $_SERVER['REQUEST_METHOD'];
		if($method != 'POST'){
			json_output(400,array('status' => 400,'message' => 'Bad request.'));
		...
		$resp = $this->LevyMod->create_levy();
		json_output($resp['status'],$resp);
	}


	public function del_levy($id)
	{
		$method = $_SERVER['REQUEST_METHOD'];
		...
		$resp = $this->LevyMod->delete_levy($id);
		json_output($resp['status'],$resp);
	}

	public function upd_levy()
	{
		$method = $_SERVER['REQUEST_METHOD'];
		if($method != 'POST'){
			json_output(400,array('status' => 400,'message' => 'Bad request.'));
